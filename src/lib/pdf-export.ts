...
export function exportIncidentToPDF(incident: Incident) {
  ...
  doc.text('Generated by VoiceWatch AI', pageWidth / 2, yPos, { align: 'center' })
  ...
  doc.text('Powered by Google Cloud • Datadog • Confluent • ElevenLabs', pageWidth / 2, doc.internal.pageSize.getHeight() - 10, { align: 'center' })
...
export function exportIncidentsReportToPDF(
  ...
  doc.setFontSize(22)
  doc.setTextColor(26, 35, 50)
  doc.text('VoiceWatch AI', pageWidth / 2, yPos, { align: 'center' })
  yPos += 8
  ...
  const filename = `voicewatch-report-${Date.now()}.pdf`
  doc.save(filename)
}
